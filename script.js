...
function handleImage(e){ const file=e.target.files[0]; if(!file) return; if(file.size > 10 * 1024 * 1024) return alert('Max image size is 10MB.'); const reader=new FileReader(); reader.onload=()=>createDraft(reader.result); reader.readAsDataURL(file); }
...
async function deployDraft(){
  ...
  if(!ok) return;
  const ad={id:String(Date.now()),image:state.draft.url,image_url:state.draft.url,link:$('adLink').value.trim(),wallet:state.wallet,amount:voucher?0:price,x:state.draft.x,y:state.draft.y,w:state.draft.w,h:state.draft.h,created_at:new Date().toISOString(),voucher_code:voucher||'',locked:true};
  await savePersistentAd(ad);
  addAdToDom(ad); deleteDraft(); closeSheet(); updateStats(); warImpact(price); alert('Ad locked. Refresh test should keep it now. Production payment verification is still required before public launch.');
}
function getAds(){ try{return JSON.parse(localStorage.getItem('jakwo_ads')||'[]')}catch{return[]} }
function setAds(ads){ localStorage.setItem('jakwo_ads',JSON.stringify(ads)); }
function normalizeAd(a){ return {...a, image:a.image || a.image_url, image_url:a.image_url || a.image, id:a.id || String(Date.now())}; }
async function savePersistentAd(ad){
  const ads=getAds().map(normalizeAd); ads.push(ad); setAds(ads);
  // Optional Supabase REST save. If table/policies are not ready, localStorage still preserves refresh on this device.
  if(cfg.SUPABASE_URL && cfg.SUPABASE_ANON_KEY){
    try{
      await fetch(`${cfg.SUPABASE_URL}/rest/v1/ads`,{
        method:'POST',
        ...
        body:JSON.stringify({image_url:ad.image_url,link:ad.link,wallet:ad.wallet,amount:ad.amount,x:ad.x,y:ad.y,w:ad.w,h:ad.h,locked:true,voucher_code:ad.voucher_code})
      });
    }catch(e){ console.warn('Supabase save skipped:', e); }
  }
}
async function loadPersistentAds(){
  let ads=getAds().map(normalizeAd).filter(a=>a.image);
  if(cfg.SUPABASE_URL && cfg.SUPABASE_ANON_KEY){
    try{
      const res=await fetch(`${cfg.SUPABASE_URL}/rest/v1/ads?select=*&order=created_at.asc`,{headers:{'apikey':cfg.SUPABASE_ANON_KEY,'Authorization':`Bearer ${cfg.SUPABASE_ANON_KEY}`}});
      if(res.ok){
        const remote=(await res.json()).map(normalizeAd).filter(a=>a.image);
        const seen=new Set(); ads=[...remote,...ads].filter(a=>{const k=(a.created_at||'')+(a.wallet||'')+(a.x||'')+(a.y||''); if(seen.has(k))return false; seen.add(k); return true;});
        setAds(ads);
      }
    }catch(e){ console.warn('Supabase load skipped:', e); }
  }
  $('adLayer').innerHTML=''; ads.forEach(addAdToDom); updateStats();
}
function addAdToDom(ad){ ad=normalizeAd(ad); const el=document.createElement('a'); el.className='war-ad locked'; el.href=ad.link||'#'; el.target='_blank'; el.rel='noreferrer'; el.style.left=ad.x+'px'; el.style.top=ad.y+'px'; el.style.width=ad.w+'px'; el.style.height=ad.h+'px'; el.style.zIndex=100+Number(String(ad.id).replace(/\D/g,'').slice(-5)||0); el.title=`${Number(ad.amount||0).toFixed(2)} USDC • ${shortWallet(ad.wallet)}`; const img=document.createElement('img'); img.src=ad.image; el.appendChild(img); $('adLayer').appendChild(el); $('emptyCallout').style.display='none'; }
function updateStats(){ const ads=getAds().map(normalizeAd).filter(a=>a.image); $('totalAds').textContent=ads.length; const vol=ads.reduce((s,a)=>s+Number(a.amount||0),0); $('usdcVolume').textContent=vol.toLocaleString(undefined,{maximumFractionDigits:2}); $('latestWar').textContent=ads.length?shortWallet(ads[ads.length-1].wallet):'None'; const top={}; ads.forEach(a=>top[a.wallet]=(top[a.wallet]||0)+Number(a.amount||0)); const tw=Object.entries(top).sort((a,b)=>b[1]-a[1])[0]; $('topWarlord').textContent=tw?shortWallet(tw[0]):'None'; $('emptyCallout').style.display=ads.length?'none':'block'; }
